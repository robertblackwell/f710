...
#define CONST_SELECT_TIMEOUT_INTERVAL_MS 500ms
 ...
 */
    ...
    void f710::F710::handle_read(const boost::system::error_code& ec, std::size_t length) const
    {
    }
    void f710::F710::start_read()
    {
     ...
     */
    void f710::F710::handle_event(js_event event, ControllerState* cstate)
    {
        switch (event.type) {
            case  JS_EVENT_INIT | JS_EVENT_BUTTON:
                RBL_LOG_FMT("js_event_init_button time: %d number: %d value: %d type: %Xh",
